...
def process_news_with_gemini(raw_news_list):
    ...
    # 주말 여부 판별 (5: 토요일, 6: 일요일)
    is_weekend = datetime.now().weekday() >= 5
    weekend_instruction = """
    현재는 주말(장 마감 이후)입니다. 
    지수 변동이나 단순 주가 등락 소식보다는 아래 주제를 최우선으로 선정해줘:
    - 한 주간의 경제 주요 이슈 요약 및 복기
    - 다음 주 예정된 주요 경제 지표 발표 및 시장 영향 전망
    - 장기적인 거시경제 정책, 산업 구조 변화, 규제 관련 심층 분석 뉴스
    - 기업의 실적 발표보다는 비즈니스 모델이나 미래 전략 관련 소식
    """ if is_weekend else ""
    ...
    [선정 기준]
    0. (평일에만 적용) 지수 변동, 환율 변동, 금리 변동을 알리는 직접적인 뉴스인가?
...
def main():
    ...
    if final_news:
        ...
        # 주말 여부 판별
        is_weekend = datetime.now().weekday() >= 5
        push_title = "📅 주말 거시경제 브리핑" if is_weekend else "📰 오늘의 주요 뉴스 업데이트"
        push_body = f"주말 동안의 핵심 소식 5개를 전해드립니다: {final_news[0]['keyword']} 외 4건" if is_weekend else f"오늘의 핵심 뉴스 5개가 도착했습니다: {final_news[0]['keyword']} 외 4건"
